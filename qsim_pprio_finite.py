...
class JobQueue:
    ...
    def __init__(self, current_time):
        ...
        # Number of low priority jobs in system (queue + server).
        self.cnt_l = 0
    ...
    def enqueue(self, job):
        ...
        # Low priority queue is full, need to kick one job out.
        if self.cnt_l >= BUF_SIZE and job.job_type > 1:
            #victim = sorted([j for j in self.q if j.job_type >= 1000000],
            victim = sorted([j for j in self.q if j.job_type >= 1],
                            key=lambda x: x.t_arrival)[0]
...
class QSim:
    ...
    def _arrivalHelper(self):
        ...
        hasDrop = self.q.enqueue(Job(self.global_jid.increment(), self.clock,
                                     service_time, job_type))
        # One job is kicked out so account for that.
        if hasDrop:
            self.num_jobs -= 1
        job = self.q.peek()
    ...
    def _completionHelper(self):
        ...
        if completed_job.job_type > 1:
            ...
            if self.t_last_complete_l != -1:
                self.inter_completion_time += self.clock - self.t_last_complete_l
            self.t_last_complete_l = self.clock
