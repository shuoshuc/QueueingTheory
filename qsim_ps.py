...
class QSim:
    def __init__(self):
        ...
        # A LIFO queue tracking the arrival times of each job, for computing the
        # *response* time later.
    ...
    def _arrivalHelper(self):
        ...
        # Find out which jobs completes first.
        job = self.q.peek()
        self.curr_jid = job.jid
        # The shortest job needs N times its size to complete due to processor sharing.
        self.t_next_completion = self.clock + job.service_time * self.q.qlen()
        self.t_next_arrival = self.clock + self._genIAT()
    ...
    def _completionHelper(self):
        ...
        if self.num_jobs > 0:
            ...
            self.t_next_completion = self.clock + next_job.service_time * self.q.qlen()
        else:
            self.curr_jid = -1
            self.t_next_completion = float("inf")
